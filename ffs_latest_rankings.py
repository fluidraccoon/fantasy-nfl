...
def ffs_latest_rankings(rank_type="draft"):
    ...
    if rank_type == "draft":
        ...
        condition = (fp_latest["ecr_type"] == "rp") & (
            fp_latest["page_type"].str.split("-").str.get(1)
            == fp_latest["pos"].str.lower()
        )
        fp_cleaned = fp_latest[condition][
            [
                "player",
                "id",
                "pos",
    ...
    # Handle "week" type
        ...
        fp_cleaned = fp_latest[
            ["player_name", "fantasypros_id", "pos", "team", "ecr", "sd", "scrape_date"]
        ]
...
def get_ff_rosters(df_rosters):
    ff_rosters = df_rosters.merge(
        (dp_playerids()[["sleeper_id", "fantasypros_id"]])
        .dropna()
        .astype(int)
        .astype(str),
        left_on="player_id",
...
def get_ff_starter_positions(league):
    ...
    # Process the data
    df_positions = (
        roster_positions.loc[roster_positions["pos"] != "BN"]
        .groupby("pos")
        .size()
        ...
        .assign(total_starters=lambda x: x["min"].sum())
    ...
    defense_starters = df_positions.loc[
        df_positions.index.isin(["IDP_FLEX", "DL", "LB", "DB"]), "min"
    ].sum()

    kdef = df_positions.loc[df_positions.index.isin(["K", "DEF"]), "min"].sum()
    ...
    for page, season in combinations:
        try:
            rankings = fetch_fp_rankings(
                page, season
            )  # Fetch the rankings using page and season
...
def ff_apply_gp_model(adp_outcomes, model_type, fp_injury_table=None):
    if model_type == "none":
        ...
        adp_outcomes = pd.merge(
            adp_outcomes, fp_injury_table, on=["pos", "rank"], how="left"
        )

    return adp_outcomes


def ffs_adp_outcomes(
    scoring_history, seasons, gp_model="none", pos_filter=["QB", "RB", "WR", "TE"]
):
    ...
    ), "scoring_history must be a DataFrame"
    ...
    sh = scoring_history[
        (scoring_history["sleeper_id"].notna()) & (scoring_history["week"] <= 17)
    ][["sleeper_id", "team", "season", "points"]]

    # Load necessary datasets
    fp_rh = fp_rankings_history.drop(columns=["page_pos"])
    dp_id = pd.DataFrame(dp_playerids()).dropna(
        subset=["sleeper_id", "fantasypros_id"]
    ...
    ao_grouped = (
        ao.groupby(["season", "pos", "rank", "fantasypros_id", "player_name"])
        .agg(
    ...
    ao_expanded = pd.DataFrame(
        {
            ...
            "rank": np.concatenate(
                [ff_rank_expand(int(r)) for r in ao_grouped["rank"]]
            ),
        }
    )
    ...
    ao_final = (
        ao_expanded.groupby(["pos", "rank", "prob_gp"])
        .agg(
            ...
            player_name=(
                "player_name",
                list,
            ),  # No need for lambda, list() works directly
            fantasypros_id=(
                "fantasypros_id",
                list,
            ),  # No need for lambda, list() works directly
    ...
    ao_final = (
        ao_final[ao_final["fantasypros_id"].notna()]
        .sort_values(by=["pos", "rank"])
...
def ffs_generate_projections_week(adp_outcomes, latest_rankings, sims, rosters=None):
    ...
    latest_rankings = latest_rankings[
        ["player", "pos", "team", "ecr", "sd", "fantasypros_id", "scrape_date"]
    ]
    ...
    rankings = latest_rankings[
        latest_rankings["fantasypros_id"].isin(rosters["fantasypros_id"])
    ]
    ...
    expanded_rankings["rank"] = np.round(
        norm.rvs(loc=expanded_rankings["ecr"], scale=expanded_rankings["sd"] / 2)
    ).astype(int)
    ...
    ps["projected_score"] = ps.apply(
        lambda row: np.random.choice(row["week_outcomes"]), axis=1
    )
...
def ffs_generate_projections(
    adp_outcomes, latest_rankings, sims=100, weeks=range(1, 15), rosters=None
):
    ...
    if not isinstance(weeks, (list, range)) or not all(
        isinstance(w, int) for w in weeks
    ):
    ...
    if not isinstance(latest_rankings, pd.DataFrame):
        raise ValueError("`latest_rankings` must be a pandas DataFrame")
    latest_rankings = latest_rankings[
        ["player", "pos", "team", "ecr", "sd", "bye", "fantasypros_id", "scrape_date"]
    ]
    ...
    ps["gp_model"] = ps.apply(
        lambda row: sample_gp_model(row["prob_gp"], n_weeks), axis=1
    )
    ...
    ps["projected_score"] = ps.apply(
        lambda row: row["projection"] * row["gp_model"] * (row["week"] != row["bye"]),
        axis=1,
    )
...
def ffs_score_rosters(projected_scores, rosters):
    ...
    roster_scores["pos_rank"] = (
        roster_scores.groupby(["roster_id", "position", "season", "week"])[
            "projected_score"
        ]
...
def ff_optimise_one_lineup(franchise_scores, lineup_constraints):
    ...
    for i, pos in enumerate(lineup_constraints["pos"]):
        ...
        constraints_matrix[row, :] = np.concatenate(
            [np.where(franchise_scores["pos"] == pos, 1, 0), [1] * min_req]
        )
        row += 1
    for i, pos in enumerate(lineup_constraints["pos"]):
    ...
    constraints_matrix[row, :] = np.concatenate(
        [
            np.where(franchise_scores["pos"].isin(["QB", "RB", "WR", "TE"]), 1, 0),
            [1] * min_req,
        ]
    )
    row += 1
    ...
    constraints_dir = (
        [">="] * len(lineup_constraints)
        + ["<="] * len(lineup_constraints)
        + ["<=", "<="]
    )

    # Right-hand side (RHS) of constraints
    constraints_rhs = np.concatenate(
        [
    ...
    if res.success:
        selected_players = np.where(res.x > 0.5)[
            0
        ]  # Since the solution is binary, 0 or 1
...
def ffs_optimise_lineups(
    ...
    best_ball=False,
    pos_filter=("QB", "RB", "WR", "TE"),
):
    # Validate inputs
    assert (
        0 <= lineup_efficiency_mean <= 1
    ), "lineup_efficiency_mean must be between 0 and 1"
    assert (
        0 <= lineup_efficiency_sd <= 0.25
    ), "lineup_efficiency_sd must be between 0 and 0.25"
    ...
    optimal_scores = pd.merge(
        roster_scores, max_lineup_constraints, left_on="position", right_on="pos"
    )
    ...
    if best_ball:
        ...
        optimal_scores_df["lineup_efficiency"] = np.random.normal(
            lineup_efficiency_mean, lineup_efficiency_sd, len(optimal_scores_df)
        )

    # Calculate actual score
    optimal_scores_df["actual_score"] = (
        optimal_scores_df["optimal_score"] * optimal_scores_df["lineup_efficiency"]
    )

    return optimal_scores_df


def ffs_summarise_week(
    ...
    team["allplay_wins"] = (
        team.groupby(["season", "week"])["actual_score"].rank(method="min") - 1
    )
    team["allplay_games"] = (
        team.groupby(["season", "week"])["actual_score"].transform("count") - 1
    )
    ...
    summary_week = pd.merge(
        schedules,
        team,
        left_on=["roster_id", "season", "gameweek"],
        right_on=["roster_id", "season", "week"],
        how="inner",
    )
    summary_week = pd.merge(
        summary_week,
    ...
    summary_week["result"] = np.select(
        [
            summary_week["team_score"] > summary_week["opponent_score"],
            summary_week["team_score"] < summary_week["opponent_score"],
            summary_week["team_score"] == summary_week["opponent_score"],
        ],
        ["W", "L", "T"],
    ...
    summary_week = summary_week.merge(
        ...
        left_on=["roster_id", "week"],
        right_on=["roster_id", "gameweek"],
        how="left",
    )
    summary_week["selected_pts"] = np.where(
        summary_week["week"] > current_gameweek,
        summary_week["team_score"],
        summary_week["points"],
    )
    summary_week["selected_win"] = np.where(
        summary_week["week"] > current_gameweek,
        np.where(
            summary_week["result"] == "W",
            1,
            np.where(summary_week["result"] == "T", 0.5, 0),
        ),
        summary_week["win"],
    )
    summary_week["selected_all_play"] = np.where(
        summary_week["week"] > current_gameweek,
        summary_week["allplay_wins"],
        summary_week["all_play"],
    )
    summary_week["selected_pp"] = np.where(
        summary_week["week"] > current_gameweek,
        summary_week["optimal_score"],
        summary_week["pp_points"],
    )
    summary_week["selected_pp_opponent"] = np.where(
        summary_week["week"] > current_gameweek,
        summary_week["optimal_opponent_score"],
        summary_week["pp_points_opponent"],
    )
    summary_week["selected_pp_win"] = np.where(
        summary_week["week"] > current_gameweek,
        np.where(
            summary_week["optimal_score"] > summary_week["optimal_opponent_score"],
            1,
            np.where(
                summary_week["optimal_score"] == summary_week["optimal_opponent_score"],
                0.5,
    ...
    summary_week["nxt_wk_win"] = np.where(
        summary_week["week"] == current_gameweek + 1, summary_week["selected_win"], 0
    )
