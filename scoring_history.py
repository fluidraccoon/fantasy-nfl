...
def dp_playerids():
    ...
    if response.status_code != 200:
        raise Exception(
            f"GitHub request failed with error: <{response.status_code}> while calling <{url_query}>"
        )
...
def nflverse_roster(season):
    ...
    ros = (
        ros.groupby(["season", "gsis_id", "sportradar_id"])
        .last()
        .reset_index()
...
def sleeper_rule_mapping(league):
    ...
    scoring_rules = pd.DataFrame(
        list(scoring_settings.items()), columns=["event", "points"]
    )
...
def ff_scoring_sleeper_conn(league, df_sleeper_rule_mapping):
    ...
    scoring_rules = pd.merge(
        scoring_rules, df_sleeper_rule_mapping, on="event", how="inner"
    )
...
def get_league_rules(scoring_rules):
    ...
    league_rules = pd.merge(
        scoring_rules,
        nflfastr_stat_mapping_sleeper,
        left_on="event",
        right_on="ff_event",
        how="left",
    )
...
def get_scoring_history(league, start_year, end_year):
    ...
    merged_data["points"] = merged_data.groupby(["season", "week", "player_id"])[
        "points"
    ].transform(lambda x: round(x.sum(), 2))
